...
class FSMSQuote {
	...
	private function add( $vars ) {
		global $wpdb;
		$vars              = FSMSQuote::prepare_vars( $vars );
		$invoice_tablename = $wpdb->prefix . "fsms_invoice";
		$result            = $wpdb->query( $wpdb->prepare( "INSERT INTO " . $invoice_tablename . " SET invoice_type='quote',
	...
	public function send_quote_to_customer_email() {
		global $wpdb;
		...
		if ( ! empty( $_POST['ok'] ) && wp_verify_nonce( $_POST['nonce'], 'fsms_send_invoice' ) ) {
			...
			$lastid   = FSMSQuote::add( $vars );
			$fileName = "viewQuote_" . $lastid . ".html";

			FSMSQuote::quoteToHtml( $invoiceDivContent, $fileName );
			...
			$response = "<div style='font-size:18px;color:green;'>Email successfully send Quote #" . esc_attr( $lastid ) . "</div>";
		}
	...
	public function quoteToHtml( $html, $fileName ) {
		$uploads_dir = trailingslashit( wp_upload_dir()['basedir'] ) . 'fsms_quotes';
	...
	function find( $id = 0 ) {
		...
		$invoices          = $wpdb->get_results( $wpdb->prepare( "SELECT INV.*, U.user_nicename FROM " . $invoice_tablename . " as INV  LEFT JOIN " . $tablename_users . " as U ON U.ID=INV.sender_id WHERE INV.invoice_type='quote' AND INV.lead_id =%d", $id ) );
	...
	function findByid( $id = 0 ) {
		...
		$invoice_tablename = $wpdb->prefix . "fsms_invoice";
		$tablename_users   = $wpdb->prefix . "users";
		$invoices          = $wpdb->get_results( $wpdb->prepare( "SELECT INV.*, U.user_nicename FROM " . $invoice_tablename . " as INV  LEFT JOIN " . $tablename_users . " as U ON U.ID=INV.sender_id WHERE INV.invoice_type='quote' AND INV.id =%d", $id ) );
