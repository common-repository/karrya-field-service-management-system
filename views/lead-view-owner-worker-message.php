...
<table>
    ...
    <tr>
        <td width="50%" valign="top">
            <table>
                <caption>Owner
                    <span onclick="showOwnerMessageBox();" class='cursorPointer'>Add message</span></caption>
                <tr id="ownerMessage" style="display:none">
                    <td valign="top">
                        <form action="admin.php?page=fsms_leads&action=add_message&id=<?php echo esc_attr( $id ); ?>"
                              method="POST">
							<?php wp_editor( '', 'message_owner' ) ?>
                            <input type="hidden" name="ok" value="1">
                            <input type="hidden" name="user_type" value="2">
                            <input name="nonce" type="hidden" name="ok" value="<?php echo $nonce; ?>">
                            <input type="submit" name="send" value="Send">
                            <input type="hidden" id="" class="message_parent_id" name="message_parent_id" value="0">
                            <input type="hidden" id="message_for_id" name="message_for_id"
                                   value="<?php echo esc_attr( $leadView->lead_owner ); ?>">
                            <input onclick="cancelOwnerMessageBox();" type="button" name="cancel" value="Cancel">
                        </form>
                    </td>
                </tr>
                <tr>
                    <td>
						<?php include FSMS_PATH . '/views/owner_message.php'; ?>
                    </td>
                </tr>
            </table>
        </td>
